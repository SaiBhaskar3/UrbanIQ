...
NON_TIME_COLS = {"city", "city name", "state", "st", "state_code",
...
def _identify_date_columns(df: pd.DataFrame) -> List[str]:
    ...
    More conservative than before to avoid treating metadata as time series.
    """
    date_cols = []
    ...
    for c in df.columns:
        ...
        # Skip clearly non-time columns
        if lower_name in NON_TIME_COLS:
            continue

        # Obvious MMM-YY or MMM-YYYY pattern
        if MONTH_COL_REGEX.match(name):
            date_cols.append(c)
            continue

        # Try parsing header as a month-year, e.g. "1 Jan 2016"
        parsed = pd.to_datetime("1 " + name, errors="coerce")
        if pd.notna(parsed):
            date_cols.append(c)

    # Fallback: if still nothing and many columns, assume everything after col 5 is time
...
def _parse_timeseries_from_row(row: pd.Series, date_cols: List[str]) -> pd.Series:
    ...
    # Fallback to integer index if still no valid dates
    if parsed.isna().all():
...
def get_real_estate_data(city: str, state: str, df_rexus: Optional[pd.DataFrame]) -> dict:
    ...
    # Try to locate columns we care about
...
def semantic_retrieve_rexus(
    ...
    if embeddings is not None and model is not None:
        try:
            ...
            # If anything goes wrong, fall back to substring matching
            pass
