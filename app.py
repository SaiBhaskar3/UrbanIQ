...
if page == "Compare":
    ...
    if st.session_state.data1 and st.session_state.data2:
        ...
        def property_card(data, title):
            ...
            st.markdown("---")
            st.write("**Latest Price (city-level):**", price_info.get("latest_price", "No data"))
            st.write("**Median Price (city-level):**", price_info.get("median_price", "No data"))
        ...
        ts_df = (
            pd.concat([ts_df1, ts_df2], axis=1)
            if not ts_df1.empty and not ts_df2.empty else ts_df1 if not ts_df1.empty else ts_df2
        )
        ...
        if not ts_df.empty:
            ...
            st.info("No city-level time series price data available.")
